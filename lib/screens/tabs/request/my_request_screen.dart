...
import 'package:tchat_app/screens/items/item_my_friend_request.dart';
...
class _MyRequestScreenState extends AccountBaseState<MyRequestScreen> with AutomaticKeepAliveClientMixin{
  ...
  Widget initListView(){
    return ListView.separated(
      ...
      itemBuilder: (context,index)=>ItemMyFriendRequest(context, listFriend[index], userModel),
  ...
  getData()async{
    ...
    await FirebaseFirestore.instance.collection(FIREBASE_FRIENDS).doc(userModel.id).collection(userModel.id).where(FRIEND_STATUS_REQUEST,isEqualTo: FRIEND_SEND_REQUEST).get().then((value) {
      if(value.size>0){
